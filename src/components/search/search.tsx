...
export default function Search() {
  ...
  return (
    <form className="relative group w-full md:w-60" onSubmit={handleSearch}>
      ...
      {query && (
        <CircleX className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-300 transition-colors duration-200 group-focus-within:text-blue-300" onClick={resetSearch} />
      )}
    </form>
  );
}